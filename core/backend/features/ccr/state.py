...
Moved from ``solvers/ccr.py`` in the engine work order (v1 O8 / v2 O12,
``docs/feature-modules-plan.md``); this module holds the mutable state
object, ``rules.py`` the ``CapRule`` implementation. ``ets/solvers/ccr.py``
remains as a re-export shim (including the ``CCR_DEFAULTS`` re-export from
``core.defaults``).
"""
